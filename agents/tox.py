...
def get_running_commands(component_details):
    ...
    component = component_details['components'][0]

    commands = [
        ['rm', '-rf', component['canonical_project_name']],
        ['git', 'clone', component['git']]
    ]

    commands.append({'cwd': '/var/tmp/%s' % component['canonical_project_name'],
                     'cmd': ['git', 'checkout', component['sha']]})
    commands.append({'cwd': '/var/tmp/%s' % component['canonical_project_name'],
                     'cmd': ['tox', '-e', 'pep8']})
    return commands
