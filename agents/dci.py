...
def _get_os_version():
    ...
    elif current_platform[0] == 'CentOS Linux':
...
def get_pre_run_commands(component_details, environment_url):
    ...
    commands = [
        ['curl', '-o', '/etc/yum.repos.d/dci.repo', '%s/%s' % (cmpnt['url'],
                                                               current_os)],
...
def main():
    ...
    pre_run_commands = get_pre_run_commands(jobinformation)
    ...
    running_commands = get_running_commands(environment_url)
    ...
    post_run_commands = get_post_run_commands(jobinformation)
