...
class DCIBaseResource(object):
    ...
    def __init__(self, session, endpoint_uri, validator_scheme):
        self._s = session
        self._end_point_with_uri = '%s/%s/%s' % (self._s.dci_cs_url,
                                                 self.API_URI, endpoint_uri)
        self._validator_scheme = validator_scheme

    def _verify_mandatory_parameters(self, action, **kwargs):
        """Ensure mandatory parameters are present else raise an exception"""
        for parameter in self._validator_scheme[action]:
    ...
    def create(self, **kwargs):
        """Create a resource"""
        self._verify_mandatory_parameters('create', **kwargs)
    ...
    def get(self, **kwargs):
        ...
        self._verify_mandatory_parameters('get', **kwargs)

        return self._s.get("%s/%s" % (self._end_point_with_uri, kwargs['id']))

    def update(self, **kwargs):
        """Update a specific resource"""
        self._verify_mandatory_parameters('update', **kwargs)
    ...
    def delete(self, **kwargs):
        ...
        self._verify_mandatory_parameters('delete', **kwargs)
