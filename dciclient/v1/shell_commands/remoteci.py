...
def create(session, name):
    utils.print_json(remoteci.RemoteCI(session).create(name=name).json())
...
def update(session, id, etag, name):
    result = remoteci.RemoteCI(session).update(_id=id, etag=etag, name=name)
...
def delete(session, id, etag):
    result = remoteci.RemoteCI(session).delete(_id=id, etag=etag)
    if result.status_code == 204:
        utils.print_json({"id": id,
...
@click.pass_obj
def show(session, id):
    result = remoteci.RemoteCI(session).get(_id=id)
    utils.print_json(result.json())
