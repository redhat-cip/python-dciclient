...
def attach_rconfiguration(context, id, name, topic_id, component_types, data):
    ...
    :param string topic_id: ID of the topic to associate this rconfiguration with
        [required]
    :param string component_types: list data to represent the overriden component_types
...
def list_rconfigurations(context, id, sort, limit, where, verbose):
    ...
    :param boolean verbose: Display verbose output
    """
    result = remoteci.list_rconfigurations(context, id=id, sort=sort, limit=limit,
                                 where=where)
    utils.format_output(result, context.format, verbose=verbose)
