...
def create(session, name):
    utils.print_json(test.Test(session).create(name=name).json())
...
def update(session, id, etag, name):
    result = test.Test(session).update(_id=id, etag=etag, name=name)
...
def delete(session, id, etag):
    result = test.Test(session).delete(_id=id, etag=etag)
    if result.status_code == 204:
        utils.print_json({"id": id,
...
@click.pass_obj
def show(session, id):
    result = test.Test(session).get(_id=id)
    utils.print_json(result.json())
