...
def update(session, id, etag, name):
    result = componenttype.ComponentType(session).update(_id=id, etag=etag,
                                                         name=name)
...
def delete(session, id, etag):
    result = componenttype.ComponentType(session).delete(_id=id, etag=etag)
    if result.status_code == 204:
        utils.print_json({"id": id,
...
@click.pass_obj
def show(session, id):
    result = componenttype.ComponentType(session).get(_id=id)
    utils.print_json(result.json())
