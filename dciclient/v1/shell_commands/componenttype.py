...
def ct_create(dci_client, name):
    utils.print_json(componenttype.ComponentType(dci_client).create(name=name).json())
...
def ct_update(dci_client, id, etag, name):
    result = componenttype.ComponentType(dci_client).update(_id=id, etag=etag, name=name)
