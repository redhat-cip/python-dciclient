...
class DciTokenAuth(AuthBase):
    ...
    def __call__(self, r):
        ...
        signature = token.gen_headers(self.client_id, self.api_secret,
...
class DciTokenContext(DciContextBase):
    def __init__(self, dci_cs_url, client_id, api_secret, max_retries=0,
                 user_agent=None):
        super(DciTokenContext, self).__init__(dci_cs_url, max_retries,
                                              user_agent)
        self.session.auth = DciTokenAuth(client_id, api_secret)
...
def build_token_context(dci_cs_url=None, dci_client_id=None,
                        dci_api_secret=None,
    ...
    if not dci_cs_url or not dci_client_id or not dci_api_secret:
        ...
        sys.exit(1)
    return DciTokenContext(dci_cs_url, dci_client_id, dci_api_secret,
                           user_agent=user_agent, max_retries=max_retries)
