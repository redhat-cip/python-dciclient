...
def dci_context(server, db_provisioning):
    context = base.DciContext('http://dci_server.com', 'admin', 'admin')
    flask_adapter = utils.FlaskHTTPAdapter(server.test_client())
    context.session.mount('http://dci_server.com', flask_adapter)
    return context


@pytest.fixture
def runner(monkeypatch, dci_context):
    base.build_dci_context = lambda **kwargs: dci_context
