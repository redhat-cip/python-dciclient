...
def test_annotate(runner, topic_id):
    ...
    jobdefinition_id = json.loads(result.output)['jobdefinition']['id']
    result = runner.invoke(['jobdefinition-annotate', '--id', jobdefinition_id,
                            '--comment', 'This is my annotation'])

    result = json.loads(result.output)
    assert result['message'] == 'Job Definition updated.'

    result = runner.invoke(['jobdefinition-show', '--id', jobdefinition_id])
