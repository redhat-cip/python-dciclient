...
def push_stack_details(context, undercloud):
    ...
    undercloud.run(
        'test -d tripleo-stack-dump || git clone https://github.com/goneri/tripleo-stack-dump',
        user='stack')
...
def run_tests(context, undercloud_ip, key_filename, user='root'):
    ...
    if undercloud.run('test -f stackrc', user='stack', success_status=(0, 1,))[0] != 0:
        msg = ' deployment failure'
        jobstate.create(context, 'failure', msg, context.last_job_id)
        return
    if undercloud.run('test -f overcloudrc', user='stack', success_status=(0, 1,))[0] != 0:
