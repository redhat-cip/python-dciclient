...
def get_jenkins_jobs(dcicontext, config, puddles, cache=None):
    dci = config['dci']
    ...
    jobdef_in_dci = json.loads(
        dcijobdef.list(
            dcicontext, dci['topic_id'], embed='jobdefinition_component'
        ).text
    )
    ...
    with tqdm(total=len(puddles) * len(job_array)) as pbar:
        for puddle, job in itertools.product(puddles, job_array):
            ...
            for dci_job in jobdef_in_dci['jobdefinitions']:
                ...
                newjob = json.loads(
                    dcijobdef.create(
                        dcicontext, job['name'], dci['topic_id']
                    ).text
                )
    ...
    jobdef_in_dci = json.loads(
        dcijobdef.list(
            dcicontext, dci['topic_id'], 'jobdefinition_component'
        ).text
    )

    # Parse all status to put the right jenkins
    # status in the right DCI jobstate
    for status, item in itertools.product(
        tqdm(temparray), jobdef_in_dci['jobdefinitions']
    ):
        ...
        args = [dcicontext, False, dci['remoteci_id'],
                dci['team_id'], jobid, status["url"]]
...
if __name__ == '__main__':
    ...
    get_jenkins_jobs(dci_context, config, puddles)
