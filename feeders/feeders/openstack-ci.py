...
def get_jenkins_jobs(dcicontext, topic_id, puddles, remoteci_id, team_id, test_id, cache=None):
    job_array = []
    views = []
    multijobs = []
    jobs = []
    ...
    print('Grabbed %s job definition in DCI' % len(jobdef_in_dci['jobdefinitions']))
    ...
    print('Processing %s potential new job definitions in DCI' % (len(puddles)*len(job_array)))
            ...
            for job in job_array:
                ...
                for dci_job in jobdef_in_dci['jobdefinitions']:
                    if dci_job['name'] == job['name']:
                        if dci_job['jobdefinition_component']['component_id'] == puddle["id"]:
                            flag = 1
                            break
                if flag == 0:
                    newjob=json.loads(dcijobdef.create(dcicontext, job['name'], topic_id).text)
    ...
    for job in job_array:
        ...
        for build in tqdm(builds):
            ...
            if puddlev:
		temparray.append({ 'name' : job['name'], 'puddle': puddlev, 'result': build['result'], 'url': build['url']})
    ...
    #Parse all status to put the right jenkins status in the right DCI jobstate
    for status in tqdm(temparray):
	for item in jobdef_in_dci['jobdefinitions']:
            if item['name'] == status['name']:
	        if item['jobdefinition_component']['component_id'] == status["puddle"]:
                    jobid = item['id']
        flag = 0
        ...
        if flag == 0:
            job = json.loads(dcijob.create(dcicontext, False, remoteci_id, team_id, jobid, status["url"]).text)
	    state = json.loads(dcijobstate.create(dcicontext, get_job_status(status['result']), status['url'], job['job']['id']).text)
            nose_url = get_nosetests_url(status["url"])
            if nose_url:
                dcifile.create(dcicontext, "nosetests.xml", get_nosetests_content(status["url"], nose_url), "application/junit", state["jobstate"]["id"], job_id=job['job']['id'])
...
if __name__ == '__main__':
    ...
    get_jenkins_jobs(dci_context, topic_id, puddles, remoteci_id, team_id, test_id)
