...
class Gerrit(object):
    def __init__(self, gerrit_server, vote=False):
        self._user = os.environ.get("GERRIT_USER") or os.getlogin()
        self._server = gerrit_server
        self._vote = vote
    ...
    def get_open_reviews(self, gerrit_project, gerrit_filter):
        ...
        reviews = subprocess.check_output(['ssh', '-xp29418', self._server,
                                           '-l', self._user, 'gerrit', 'query',
                                           '--format=json',
                                           gerrit_filter])
    ...
    def get_last_patchset(self, review_number):
        ...
        lpatchset = subprocess.check_output([
            'ssh', '-xp29418', '-l', self._user,
            self._server, 'gerrit', 'query',
            '--format=JSON',
