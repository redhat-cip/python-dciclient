...
def register_github_commit(
    ...
    return dci_client.get('/components/' + component['id']).json()


def register_delorean_snapshot(
        dci_client,
        componenttype,
        url,
        canonical_project_name):
...
def get_gerrit_review_as_component(
    ...
    for patchset in gerrit.list_open_patchsets(gerrit_project, gerrit_filter):
        ...
        component = dci_client.find_or_create_or_refresh(
            '/components', version_data, unicity_key=['sha'])
        yield dci_client.get('/components/' + component['id']).json()
...
def main():
    ...
    base_components = [
        ...
        register_delorean_snapshot(
            ...
            url=(
                ...
                'centos-kilo/current/delorean-rdo-management.repo'))]
    ...
    khaleesi_master = register_github_commit(
        ...
        github_account='redhat-openstack',
        branch_name='master')
    dci_client.create_jobdefinition(
        test,
        'git' + khaleesi_master['sha'] + '/' + khaleesi_master['name'],
        base_components + [khaleesi_master])

    # Push the last Gerrit review
    for component in get_gerrit_review_as_component(
            dci_client,
            componenttypes['git_commit'],
            config_file['gerrit']['canonical_project_name'],
            config_file['gerrit']['git_url'],
            config_file['gerrit']['server'],
            config_file['gerrit']['project'],
            config_file['gerrit']['filter']):
        dci_client.create_jobdefinition(
            test,
            component['name'] + ' / ' + config_file['test']['name'],
